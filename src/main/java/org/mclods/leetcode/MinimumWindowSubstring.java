...
public class MinimumWindowSubstring {
    ...
    public String minWindow(String s1, String s2) {
        ...
        // s1, s2 includes uppercase and lowercase letters
        ...
        int start = 0, end = -1, parsedChars = 0, minWindowLen = n1 + 1;
            ...
            if (startIsValid) {
                ...
                if (s2CharFreq[endingCharIndex] > 0) {
                    ...
                    if (parsedChars == neededChars) {
                        ...
                        // Check and adjust parsed chars
                        if (window[startingCharIndex] < s2CharFreq[startingCharIndex]) {
                            parsedChars--;
                        }
                ...
                // s2 contains the starting character
                if (s2CharFreq[startingCharIndex] > 0) {
                    if(start <= end) {
                        // Check if new window already contains all parsedChars or not
                        if (parsedChars == neededChars) {
                            int windowLen = end - start + 1;
